...
const InvoiceSettings = () => {
  ...
  const addItem = () => {
    setForm(f => ({ ...f, items: [ ...(f.items || []), { description: 'New Item', quantity: 1, unitPrice: 0, total: 0, _id: String(Date.now()) } ] }));
  };
  ...
  const modalStyle = {
    background: '#fff', padding: 16, borderRadius: 8, width: '90%', maxWidth: 900, maxHeight: '90vh', overflowY: 'auto'
  };
  ...
  return (
      ...
      <div className="medicine-page-header">
        <div className="header-title-group">
            <div>
                ...
                <p style={{margin: 0}}>Manage invoices: create, search, edit, delete and view stats.</p>
            </div>
        </div>
    ...
    <div className="filter-bar" style={{ marginBottom: '1rem' }}>
        <div className="filter-group">
            ...
            <input className="invoice-input" placeholder="Invoice #, notes..." value={query} onChange={e=>setQuery(e.target.value)} />
        </div>
        <div className="filter-group">
            <label>Patient ID</label>
            <input className="invoice-input" placeholder="Patient ID..." value={filters.patient} onChange={e=>setFilters({...filters, patient: e.target.value})} />
        </div>
                ...
                <div key={inv._id || inv.id} style={{ borderBottom:'1px solid #f0f0f0', padding:8, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                  <div>
                    <div style={{ fontWeight:700, display:'flex', gap:8, alignItems:'center' }}>
                      ...
                      <span className={statusBadgeClass(inv.status)}>{inv.status || 'Unpaid'}</span>
                    </div>
                  ...
                  <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                    <button className='btn' onClick={()=>handleEdit(inv)} title="Edit" aria-label={`Edit invoice ${inv.invoiceNumber || inv._id || ''}`}>
                      <FaEdit />
                    </button>
                    <button className='btn-danger' onClick={()=>handleDelete(inv._id || inv.id)} title="Delete" aria-label={`Delete invoice ${inv.invoiceNumber || inv._id || ''}`}>
                      <FaTrash />
                    </button>
                  </div>
                </div>
      ...
      {invoiceModalOpen && (
        <div style={modalOverlayStyle} onClick={() => setInvoiceModalOpen(false)}>
            <div style={modalStyle} onClick={e => e.stopPropagation()}>
                <h3>{editing ? 'Edit Invoice' : 'Create Invoice'}</h3>
                <div style={{ display:'grid'}}>
                    <input className="invoice-input" placeholder="Invoice Number" value={form.invoiceNumber} onChange={e=>setForm({...form, invoiceNumber: e.target.value})} />
                    <input className="invoice-input" placeholder="Patient ID" value={form.patient} onChange={e=>setForm({...form, patient: e.target.value})} />
                    <input className="invoice-input" placeholder="Appointment ID (optional)" value={form.appointment} onChange={e=>setForm({...form, appointment: e.target.value})} />
                    <input className="invoice-input" placeholder="Doctor ID (optional)" value={form.doctor} onChange={e=>setForm({...form, doctor: e.target.value})} />
                    <div className="invoice-form-card">
                        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:8 }}>
                        <div style={{ fontWeight:600 }}>Items</div>
                        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                            <button onClick={()=>setItemsModalOpen(true)} className="btn btn-ghost btn-small">Open Items</button>
                        </div>
                        </div>
                        <div style={{ marginBottom: 8, color:'#666' }}>{(form.items || []).length} item(s) • Click "Open Items" to edit details</div>
                    </div>
                    <input className="invoice-input" placeholder="Notes (optional)" value={form.notes} onChange={e=>setForm({...form, notes: e.target.value})} />
                    <div style={{ display:'grid',gridTemplateColumns:'150px 100px 100px', gap:8, width:'100%' }}>
                    <input className="invoice-input" placeholder="Due Date" type='date' value={form.dueDate} onChange={e=>setForm({...form, dueDate: e.target.value})} />
                    <input className="invoice-input" placeholder="Tax" value={form.tax} onChange={e=>setForm({...form, tax: Number(e.target.value)})} />
                    <input className="invoice-input" placeholder="Discount" value={form.discount} onChange={e=>setForm({...form, discount: Number(e.target.value)})} />
                    </div>
                    ...
                    <div className="invoice-summary">
                        ...
                        <div className="muted">Tax: <strong>{Number(form.tax || 0).toFixed(2)}</strong> • Discount: <strong>{Number(form.discount || 0).toFixed(2)}</strong></div>
                        <div style={{ marginTop:6, fontSize:16 }}>Total: <strong>{computedTotal.toFixed(2)}</strong></div>
                    </div>
                    <div style={{ display:'flex', gap:8 }}>
                        <button className="btn btn-primary" onClick={handleCreateOrUpdate}>{editing ? 'Update' : 'Create'}</button>
                        <button className="btn btn-ghost" onClick={() => setInvoiceModalOpen(false)}>Cancel</button>
                    </div>
                    </div>
                </div>
            </div>
        ...
        <div style={modalOverlayStyle} onClick={()=>setItemsModalOpen(false)}>
          <div style={modalStyle} onClick={e=>e.stopPropagation()}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
              ...
              <div>
                <button className="btn btn-ghost" onClick={()=>setItemsModalOpen(false)}>Close</button>
              </div>
            </div>
            <div style={{ marginBottom:12 }}>
              <button className="btn btn-primary btn-small" onClick={addItem}>+ Add Item</button>
            </div>
            <table className="invoice-items-table" style={{ width:'100%', marginBottom: 8 }}>
              <thead>
              ...
              <tbody>
                ...
                {(form.items || []).map((it, idx) => (
                  <tr key={it._id || idx}>
                    <td style={{ padding: 6 }}>
                      <input className="invoice-input" value={it.description} onChange={e=>updateItem(idx, 'description', e.target.value)} />
                    </td>
                    <td style={{ padding: 6 }}>
                      <input className="invoice-input small-number" type="number" min={0} value={it.quantity} onChange={e=>updateItem(idx, 'quantity', e.target.value)} />
                    </td>
                    <td style={{ padding: 6 }}>
                      <input className="invoice-input small-number" type="number" min={0} value={it.unitPrice} onChange={e=>updateItem(idx, 'unitPrice', e.target.value)} />
                    </td>
                    <td style={{ padding: 6, textAlign: 'right' }}>{(Number(it.total) || 0).toFixed(2)}</td>
                    <td style={{ padding: 6 }}>
                      <button className="btn btn-danger btn-small" onClick={()=>removeItem(idx)}>Remove</button>
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <div style={{ display:'flex', justifyContent:'flex-end', gap:8 }}>
              <button className="btn btn-ghost" onClick={()=>setItemsModalOpen(false)}>Done</button>
            </div>
          </div>
        </div>
      )}
